...
class _StatisticsState extends State<Statistics>{
  ...
  Widget build(BuildContext context) {
    if(result != null){
      return DataTable(
          ...
          sortAscending: sort,
          columns: [
            DataColumn(label: Text(AppLocalizations.of(context).translate("county_")), numeric: false, onSort: (columnIndex, ascending){      
            ...
            DataColumn(label: Text(AppLocalizations.of(context).translate("infected_")), numeric: true, onSort: (columnIndex, ascending){
            ...
            DataColumn(label: Text(AppLocalizations.of(context).translate("dead_")), numeric: true, onSort: (columnIndex, ascending){
            ...
            DataColumn(label: Text(AppLocalizations.of(context).translate("recovered_")), numeric: true, onSort: (columnIndex, ascending){
          ...
          DataRow(cells: [
            DataCell(Text(totalAreas.toString(), style: TextStyle(fontWeight: FontWeight.bold),)),
            DataCell(Text(totalInfected.toString(),style: TextStyle(fontWeight: FontWeight.bold),)),
            DataCell(Text(totalDead.toString(),style: TextStyle(fontWeight: FontWeight.bold),)),
            DataCell(Text(totalRecovered.toString(),style: TextStyle(fontWeight: FontWeight.bold),)),
          ])]);
    }else{
      return Container(
        color: Colors.white,
  ...
  onSortCoulumCount(int columnIndex,bool ascending){  
    if(columnIndex == 0){
      if(ascending){
        areas.sort((a, b) => a["county"].toLowerCase().compareTo(b["county"].toLowerCase()));
      }else{
        areas.sort((a, b) => b["county"].toLowerCase().compareTo(a["county"].toLowerCase()));
      }    
    }
  }
